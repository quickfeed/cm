...
{{- $col6Width := len "Effort" }}
{{- $col7Width := len "Deadline" }}
{{- range $i, $a := $labs }}
  ...
  {{- if gt (len $a.Effort) $col6Width }}{{ $col6Width = len $a.Effort }}{{- end }}
  {{- if gt (len $a.ShortDeadline) $col7Width }}{{ $col7Width = len $a.ShortDeadline }}{{- end }}
{{- end }}

| {{ padRight " Lab " $col1Width }} | {{ padRight "Topic" $col2Width }} | {{ padRight "Grading" $col3Width }} | {{ padRight "Approval" $col4Width }} | {{ padRight "Submission" $col5Width }} | {{ padRight "Effort" $col6Width }} | {{ padRight "Deadline" $col7Width }} |
| {{ padRight ":---:" $col1Width }} | {{ padRight (separator $col2Width) $col2Width }} | {{ padRight (separator $col3Width) $col3Width }} | {{ padRight (separator $col4Width) $col4Width }} | {{ padRight (separator $col5Width) $col5Width }} | {{ padRight (separator $col6Width) $col6Width }} | {{ padRight (separator $col7Width) $col7Width }} |
{{- range $index, $a := .}}
| {{ center (printf "%v" $a.Order) $col1Width }} | {{ padRight (printf "[%s][%v]" $a.Title $a.Order) $col2Width }} | {{ padRight $a.Grading $col3Width }} | {{ padRight $a.ApproveType $col4Width }} | {{ padRight $a.SubmissionType $col5Width }} | {{ padRight $a.Effort $col6Width }} | {{ padRight $a.ShortDeadline $col7Width }} |
{{- end}}
{{range $index, $a := .}}
